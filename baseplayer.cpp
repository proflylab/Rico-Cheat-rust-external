...
Vector3 BasePlayer::get_bone_position ( CSharedMemComms* drv, int bone, uintptr_t transform )
{
	...
	return drv->get_position_injected<Vector3> ( bones );
}
...
{
	uintptr_t m_pLocalTeam = drv->read<uintptr_t> ( reinterpret_cast<uintptr_t>( game::m_pLocalplayer ) + 0x540 ); ASSERT ( m_pLocalTeam ); // clientTeam
