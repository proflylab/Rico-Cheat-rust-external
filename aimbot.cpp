...
bool find_best_target ( )
{
	...
	for ( const auto& entry : game::players )
	{
		...
		if ( game::cache[player->get_uid ( mist::aim.get ( ) )].m_bFriend == true && var::aim::ignore_friends )
			continue;
	...
	if ( var::aim::hitbox == 3 && !data::aimbot::m_bLocked )
	{
		do {
			...
			float smallest = (std::min)( { distance1, distance2, distance3 } );
...
void smooth_angle ( Vector2& angles, Vector2& localViewAngles )
{
	...
	normalize ( delta );
	float smoothing_value = 5.5f * ( var::aim::smoothing / 100.f );
	...
	// fast end
	coeff = powf ( coeff, 2.f ) * 80.f / var::aim::smoothing;
...
void update_wpn_info ( )
{
	...
	if ( (uintptr_t)m_pWeapon != (uintptr_t)game::m_rLocalInfo.m_pWeapon || m_szBullet != game::m_rLocalInfo.m_szBulletType || (uintptr_t)m_pWeapon != last_weapon )
	{
		...
		uintptr_t mag = mist::aim->read<uintptr_t> ( baseProjectile + 0x2A0 );
		...
		if ( itemModProjectile )
		{
			...
			if ( projectile )
			{
				...
				if ( var::misc::thickness )
					mist::aim->write<float> ( projectile + 0x2C, var::misc::flthickness );
			}

			if ( var::misc::noshotgunspread )
			...
			for ( int h = 0; h < childrenCount; h++ )
			{
				...
				if ( strcmp ( entityType.c_str ( ), _ ( "ProjectileWeaponMod" ) ) == 0/* && mist::other->rpm<bool>(childEntity + rust::projectileVelocityModifier)*/ )
				{
					...
					ProjectileWeaponModModifier projVelModif = mist::aim->read<ProjectileWeaponModModifier> ( childEntity + 0x160 );
			...
			m_flVelocityScale = mist::aim->read<float> ( baseProjectile + 0x26C );
		}
		else
		{
...
bool process_aim_data ( )
{
	...
	if ( data::aimbot::m_vecPosition.empty ( ) || !( GetAsyncKeyState ( var::aim::aim_key ) ) )
	{
		data::aimbot::m_bLocked = false;
		return false;
	}
...
void aimbot::thread ( )
{
	...
	float stage1 = static_cast<float>( abs ( tan ( ( var::aim::aim_fov * ( M_PI / 180.f ) ) / 2 ) ) );
